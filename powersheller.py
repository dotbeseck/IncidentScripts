...
def fetch_cmdlet_from_url(cmdlet, base_url):
    ...
    try:
        ...
        print(f"{Fore.YELLOW}Warning: Error fetching info for {cmdlet} from {base_url}: {str(e)}")
        result_queue.put((cmdlet, None))
...
def fetch_cmdlet_info(cmdlet):
    ...
    if '-' in cmdlet and not cmdlet.startswith(('Get-', 'Set-', 'New-', 'Remove-', 'Invoke-', 'ConvertTo-', 'ConvertFrom-')):
...
def replace_variables(powershell_script, variables):
    ...
    for var_name, var_value in variables.items():
        ...
        powershell_script = re.sub(rf"\${var_name}\b", var_value_escaped, powershell_script)
...
def fetch_dotnet_info(dotnet_item):
    ...
    return dotnet_info.get(dotnet_item, f"No detailed information available for {dotnet_item}")
...
def analyze_variable_content(var_name, var_value, powershell_script):
    ...
    if re.search(rf"\${var_name}.*(Start-Process|Invoke-Expression)", powershell_script):
        usage_analysis.append("Used in command execution")
    
    if re.search(rf"\${var_name}.*(New-ItemProperty|Set-ItemProperty).*HKCU|HKLM", powershell_script):
...
def check_mitre_attack_techniques(powershell_script):
    ...
    if re.search(r'(Get-Process\s+lsass|Out-Minidump|Invoke-Mimikatz)', powershell_script, re.IGNORECASE):
        techniques.append(("T1003", "Credential Dumping: Possible attempt to dump credentials"))
    
    # T1112 - Modify Registry
    if re.search(r'(New-ItemProperty|Set-ItemProperty|Remove-ItemProperty)', powershell_script):
    ...
    if re.search(r'(Get-WmiObject\s+Win32_OperatingSystem|Get-ComputerInfo)', powershell_script, re.IGNORECASE):
        techniques.append(("T1082", "System Information Discovery: Gathering system information"))
    
    # T1140 - Deobfuscate/Decode Files or Information
    if re.search(r'(FromBase64String|System\.Text\.Encoding\.UTF8\.GetString)', powershell_script):
        techniques.append(("T1140", "Deobfuscate/Decode Files or Information: Decoding Base64 or encoded content"))
    
    # T1105 - Ingress Tool Transfer
    if re.search(r'(Invoke-WebRequest|Net\.WebClient|Start-BitsTransfer)', powershell_script):
        techniques.append(("T1105", "Ingress Tool Transfer: Downloading content from the internet"))
    
    # T1053 - Scheduled Task/Job
    if re.search(r'(Register-ScheduledTask|New-ScheduledTaskAction)', powershell_script):
        techniques.append(("T1053", "Scheduled Task/Job: Creating or modifying scheduled tasks"))
    
    # T1036 - Masquerading
    if re.search(r'(Set-ItemProperty.*\.exe|Rename-Item.*\.exe)', powershell_script):
        techniques.append(("T1036", "Masquerading: Possible attempt to disguise executables"))
    
    # T1070 - Indicator Removal on Host
    if re.search(r'(Clear-EventLog|Remove-Item\s+.*\\.*\.log)', powershell_script):
        techniques.append(("T1070", "Indicator Removal on Host: Clearing logs or deleting files"))
    
    # T1027 - Obfuscated Files or Information
    if re.search(r'(ConvertTo-SecureString|-join|ForEach|%{\$_})', powershell_script):
        techniques.append(("T1027", "Obfuscated Files or Information: Possible obfuscation techniques"))
    
    # T1218 - Signed Binary Proxy Execution
    if re.search(r'(rundll32\.exe|regsvr32\.exe|certutil\.exe)', powershell_script, re.IGNORECASE):
        techniques.append(("T1218", "Signed Binary Proxy Execution: Use of system binaries to proxy execution"))
    
    # T1055 - Process Injection
    if re.search(r'(VirtualAlloc|WriteProcessMemory|CreateRemoteThread)', powershell_script):
        techniques.append(("T1055", "Process Injection: Possible use of process injection techniques"))
    ...
    if re.search(r'(Set-MpPreference\s+-DisableRealtimeMonitoring\s+\$true|New-NetFirewallRule)', powershell_script, re.IGNORECASE):
        techniques.append(("T1562", "Impair Defenses: Attempting to disable security controls"))
    
    # T1547 - Boot or Logon Autostart Execution
    if re.search(r'(New-ItemProperty.*Run|New-ItemProperty.*RunOnce)', powershell_script, re.IGNORECASE):
        techniques.append(("T1547", "Boot or Logon Autostart Execution: Adding autostart entries"))
    
    # T1552 - Unsecured Credentials
    if re.search(r'(Get-ChildItem.*\.txt|Select-String\s+-Pattern\s+password)', powershell_script, re.IGNORECASE):
        techniques.append(("T1552", "Unsecured Credentials: Possible search for credentials in files"))
    
    # T1018 - Remote System Discovery
    if re.search(r'(Get-NetNeighbor|nbtstat\s+-n|net\s+view)', powershell_script, re.IGNORECASE):
        techniques.append(("T1018", "Remote System Discovery: Enumerating network systems"))
...
def check_evasion_techniques(powershell_script):
    ...
    if re.search(r'\-[eE][nN][cC][oO][dD][eE][dD][cC][oO][mM][mM][aA][nN][dD]', powershell_script):
        evasion_techniques.append(("Use of encoded commands", Fore.RED))
    
    # Check for execution policy bypass
    if re.search(r'-[eE]x[eE]c[uU]t[iI]on[pP]ol[iI]cy\s+[bB]y[pP]ass', powershell_script):
    ...
    for alias, cmdlet in aliases.items():
        if re.search(rf'\b{re.escape(alias)}\b', powershell_script, re.IGNORECASE):
            evasion_techniques.append((f"Use of alias '{alias}' for {cmdlet}", Fore.YELLOW))
    
    # Check for string manipulation to construct commands
    if re.search(r'\([\'"][^\'"]+[\'"]\s*\+\s*[\'"][^\'"]+[\'"]\)', powershell_script):
        evasion_techniques.append(("String concatenation used to construct commands", Fore.YELLOW))
    
    # Check for use of environment variables to obfuscate paths
    if re.search(r'\$env:(?:TEMP|APPDATA|PROGRAMDATA|USERPROFILE|WINDIR)', powershell_script, re.IGNORECASE):
        evasion_techniques.append(("Use of environment variables to obfuscate paths", Fore.YELLOW))
    
    # Check for use of Invoke-Expression with variables
    if re.search(r'Invoke-Expression\s*\$', powershell_script, re.IGNORECASE):
        evasion_techniques.append(("Use of Invoke-Expression with variables (potential dynamic execution)", Fore.RED))
    
    # Check for Base64 encoded strings
    if re.search(r'[A-Za-z0-9+/]{50,}={0,2}', powershell_script):
        evasion_techniques.append(("Presence of likely Base64 encoded data", Fore.YELLOW))
...
def check_malware_techniques(powershell_script):
    ...
    if re.search(r'(Invoke-WebRequest|DownloadString).*Invoke-Expression', powershell_script, re.IGNORECASE):
    ...
    if re.search(r'-version\s+2', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Attempt to use PowerShell version 2 (downgrade attack)", Fore.RED))
    
    # Check for disabling security features
    if re.search(r'Set-MpPreference\s+-DisableRealtimeMonitoring\s+\$true', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Attempt to disable real-time monitoring", Fore.RED))
    
    # Check for common malware file operations
    if re.search(r'(New-Object\s+System\.IO\.FileStream|\.Write\(.*\[Convert\]::FromBase64String)', powershell_script, re.IGNORECASE):
    ...
    if re.search(r'(New-ScheduledTask|Register-ScheduledTask)', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Creation of scheduled tasks (possible persistence mechanism)", Fore.YELLOW))
    ...
    if re.search(r'(VirtualAlloc|WriteProcessMemory|CreateRemoteThread)', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Use of memory manipulation functions (possible injection technique)", Fore.RED))
    
    # Check for use of common keylogging techniques
    if re.search(r'(GetAsyncKeyState|GetForegroundWindow|GetWindowText)', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Use of API calls commonly associated with keylogging", Fore.RED))
    
    # Check for use of common sandbox evasion techniques
    if re.search(r'(Get-WmiObject\s+Win32_ComputerSystem|Get-WmiObject\s+Win32_LogicalDisk)', powershell_script, re.IGNORECASE):
        malware_techniques.append(("Gathering system information (possible sandbox evasion)", Fore.YELLOW))
...
def check_persistence_techniques(powershell_script):
    ...
    if re.search(r'(New-CimInstance|Set-WmiInstance).*EventFilter', powershell_script, re.IGNORECASE):
        persistence_techniques.append(("WMI event subscription (possible fileless persistence)", Fore.RED))
    
    # Check for scheduled tasks
    if re.search(r'(Register-ScheduledTask|New-ScheduledTaskAction)', powershell_script, re.IGNORECASE):
        persistence_techniques.append(("Creation or modification of scheduled tasks", Fore.RED))
...
def check_windows_exe_mimicry(powershell_script):
    ...
    common_exes = [
        'svchost', 'lsass', 'csrss', 'winlogon', 'explorer', 'smss', 'wininit', 'mmc',
        'conhost', 'taskmgr', 'dwm', 'notepad', 'cmd', 'powershell', 'rundll32', 'regsvr32',
        'services', 'spoolsv', 'dllhost', 'logonui', 'wuauclt', 'vssvc', 'client32'
    ]
    ...
    if re.search(r'New-Item.*\.exe', powershell_script, re.IGNORECASE):
        for exe in common_exes:
            if re.search(rf'New-Item.*{re.escape(exe)}\.exe', powershell_script, re.IGNORECASE):
                mimicry_attempts.append((f"Creation of file mimicking {exe}.exe", Fore.RED))
