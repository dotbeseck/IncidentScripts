...
def check_windows_exe_mimicry(powershell_script):
    ...
    common_exes = [
        "svchost", "lsass", "csrss", "winlogon", "explorer", "smss", "wininit",
        "mmc", "conhost", "taskmgr", "dwm", "notepad", "cmd", "powershell",
        "rundll32", "regsvr32", "services", "spoolsv", "dllhost", "logonui",
        "wuauclt", "vssvc", "client32",
    ]
        ...
        for var_name in executed_vars:
            ...
            if len(executed_content) > 50 and (
                ...
                try:
                    ...
                    secondary_malware = check_malware_techniques(executed_content)
                    if secondary_malware:
                        print(f"{Fore.RED}Nested Malware Techniques:{Style.RESET_ALL}")
                        for technique, color in secondary_malware:
                            print(f"{color}- {technique}")
                except Exception as e:
...
# Common PowerShell cmdlets and their descriptions
common_cmdlets = {
...
def fetch_dotnet_info(dotnet_item):
    """Get information about .NET classes and methods"""
...
def analyze_variable_content(var_name, var_value, powershell_script):
    ...
    if re.search(rf"\${var_name}.*New-ItemProperty", powershell_script):
        analysis.append("Used to create or modify registry values")

    if re.search(rf"\${var_name}.*Set-ItemProperty", powershell_script):
        analysis.append("Used to set registry values")
    
    # Check for commands or expressions
    ...
    # Check for split and convert operations
...
def detect_mitre_atomic(powershell_script):
    """Detect MITRE ATT&CK techniques based on predefined patterns"""
